...
namespace dbcppp::DBCX3::Grammar
{
    ...
    static const rule<struct TagNetworkElement, network_element_t> network_element("NetworkElement");

    static const auto nodes_line_def = nodes;

    static const auto network_element_def =
        value_table
        | message
        | message_transmitter
    ...
    static const rule<struct TagNetworkContainer, G_NetworkContainer> network_container("NetworkContainer");

    static const auto network_container_def =
          version
        > -new_symbols
        > bit_timing
        > nodes
    ...
    BOOST_SPIRIT_DEFINE(network_element);
    BOOST_SPIRIT_DEFINE(network_container);
    ...
    struct TagNetworkElement                : error_handler, annotate_on_success {};
...
struct NetworkBuilder : boost::static_visitor<> {
    ...
    void operator()(const dbcppp::DBCX3::AST::G_BitTiming& bt) { net.bit_timing = bt; }
    void operator()(const std::vector<dbcppp::DBCX3::AST::G_Node>& nodes) {
        net.nodes.insert(net.nodes.end(), nodes.begin(), nodes.end());
    }
    void operator()(const dbcppp::DBCX3::AST::G_ValueTable& vt) { net.value_tables.push_back(vt); }
    void operator()(const dbcppp::DBCX3::AST::G_Message& msg) { net.messages.push_back(msg); }
    void operator()(const dbcppp::DBCX3::AST::G_MessageTransmitter& mt) { net.message_transmitters.push_back(mt); }
    void operator()(const dbcppp::DBCX3::AST::G_EnvironmentVariable& ev) { net.environment_variables.push_back(ev); }
    void operator()(const dbcppp::DBCX3::AST::G_EnvironmentVariableData& evd) { net.environment_variable_datas.push_back(evd); }
    void operator()(const dbcppp::DBCX3::AST::G_SignalType& st) { net.signal_types.push_back(st); }
    void operator()(const dbcppp::DBCX3::AST::G_Comment& c) { net.comments.push_back(c); }
    void operator()(const dbcppp::DBCX3::AST::G_AttributeDefinition& ad) { net.attribute_definitions.push_back(ad); }
    void operator()(const dbcppp::DBCX3::AST::G_Attribute& a) { net.attribute_defaults.push_back(a); }
    void operator()(const dbcppp::DBCX3::AST::variant_attribute_t& va) { net.attribute_values.push_back(va); }
    void operator()(const dbcppp::DBCX3::AST::G_ValueDescriptionSigEnvVar& vd) { net.value_descriptions_sig_env_var.push_back(vd); }
    void operator()(const dbcppp::DBCX3::AST::G_SignalGroup& sg) { net.signal_groups.push_back(sg); }
    void operator()(const dbcppp::DBCX3::AST::G_SignalExtendedValueType& evt) { net.signal_extended_value_types.push_back(evt); }
